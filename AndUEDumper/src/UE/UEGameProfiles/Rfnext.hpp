...
class RfnextProfile : public IGameProfile
{
public:
    RfnextProfile() = default;
    ...
    uintptr_t GetGUObjectArrayPtr() const override
    {
        std::vector<std::pair<std::string, int>> idaPatterns = {
            {"6D 39 3E 97 A0 4E 02 B0 00 F4 47 F9", 0x4},
        ...
        for (const auto &it : idaPatterns)
        {
            ...
            if (adrl != 0)   return GetUEVars()->GetBaseAddress()+0x99FAB60;
        }
